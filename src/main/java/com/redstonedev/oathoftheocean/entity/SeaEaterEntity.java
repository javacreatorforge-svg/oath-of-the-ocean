...
public class SeaEaterEntity extends Monster implements IAnimatable {
        ...
        if (soundCooldown <= 0) {
            if (nearest != null && DeepWaterCheck.isPlayerNearDeepOcean(nearest)) {
                this.level.playSound(null, this.getX(), this.getY(), this.getZ(),
                        ModSounds.SEA_EATER_IDLE.get(), SoundSource.HOSTILE, 1.3F, 1.0F);
            }
    ...
    @Override protected SoundEvent getHurtSound(DamageSource s) { return ModSounds.SEA_EATER_IDLE.get(); }
    @Override protected SoundEvent getDeathSound()              { return ModSounds.SEA_EATER_IDLE.get(); }
